...
public class DictUtils {
    ...
    public  String getDicText(String dicCode,String dicItemCode){
        String  dicText = "";
        List<DictModel> dictModelList = sysBaseAPI.queryDictItemsByCode(dicCode);
        if(dictModelList !=null && !dictModelList.isEmpty()){
            for(int i=0;i<dictModelList.size();i++){
                if(dictModelList.get(i).getValue().equals(dicItemCode)){
                    dicText = dictModelList.get(i).getText();
                    break;
                }
            }
        }
