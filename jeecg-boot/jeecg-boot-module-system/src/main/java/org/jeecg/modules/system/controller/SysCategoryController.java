...
public class SysCategoryController {
	...
	public Result<IPage<SysCategory>> queryPageList(SysCategory sysCategory,
	 ...
	 public Result<List<SysCategory>> queryList(SysCategory sysCategory,HttpServletRequest req) {
		 if(oConvertUtils.isEmpty(sysCategory.getPid())){
			 sysCategory.setPid("0");
		 }
		 QueryWrapper<SysCategory> queryWrapper = new QueryWrapper<SysCategory>();
		 ...
		 List<SysCategory> categoryList = sysCategoryService.list( queryWrapper);
   ...
   */
    ...
    @RequestMapping(value = "/loadTreeRoot", method = RequestMethod.GET)
   	public Result<List<TreeSelectModel>> loadTreeRoot(@RequestParam(name="async") Boolean async,@RequestParam(name="pcode") String pcode) {
   		Result<List<TreeSelectModel>> result = new Result<List<TreeSelectModel>>();
   		try {
   			List<TreeSelectModel> ls = this.sysCategoryService.queryListByCode(pcode,false);
   			if(!async) {
	 ...
	 /**
	  * 加载推荐首页的一级节点/如果是同步 则所有数据  -- 知识库专题
	  */
	 @RequestMapping(value = "/loadTreeRootRecommend", method = RequestMethod.GET)
	 public Result<List<TreeSelectModel>> loadTreeRootRecommend(@RequestParam(name="async") Boolean async,@RequestParam(name="pcode") String pcode) {
		 Result<List<TreeSelectModel>> result = new Result<List<TreeSelectModel>>();
		 try {
			 List<TreeSelectModel> ls = this.sysCategoryService.queryListByCode(pcode,true);
			 if(!async) {
				 loadAllCategoryChildren(ls);
			 }
			 result.setResult(ls);
