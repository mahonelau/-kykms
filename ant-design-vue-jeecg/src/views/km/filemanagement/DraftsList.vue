    ...
    <div>
      ...
      <a-table
        ...
        <span slot="action" slot-scope="text, record">
           <a-space>
             <a-popover content="提交审核">
              <a-popconfirm
                title="你是否将文件提交审核?"
                ok-text="确定"
                cancel-text="取消"
                @confirm="confirm(record)">
              <a-icon type="audit" title="提审" href="#" :style="{ fontSize: '18px', color: 'orange', }"/>
              </a-popconfirm>
             </a-popover>
...
<!--              </a-menu-item>-->
        ...
        <span slot="docTitle" slot-scope="text,record">
          ...
          <span v-else @click="showDrawer(record,true)" :title ="[ record.fileType + '文件-大小:'+ record.fileSize +' B'] "><a style="color: #303133">{{ record.title}}</a></span>
        </span>
...
<script>
  ...
  import { httpPostAction ,downloadFileName } from '@/api/manage'
  import Vue from 'vue'
  ...
  export default {
    ...
    methods: {
      ...
      confirm(record) {
        ...
        let httpurl  = this.url.submitAuditUrl;
        let method = 'put';
